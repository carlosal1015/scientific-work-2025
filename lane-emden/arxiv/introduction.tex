...
Consider the system of stellar structure equations.
\begin{equation*}
	\left.\begin{aligned}
...
To obtain numerical solutions for the Lane-Emden equation using efficient and precise methods, we convert the second-order ODE to a first-order system in two dimensions.
This formulation allows us to apply the existence and uniqueness theorem for the Lane-Emden-type equation~\cite{Biles2002}, which relies on Carathéodory's existence theorem.
% TODO: Agregar una referencia sobre el teorema de existencia de Carathéodory.
Furthermore, we use the Adomian decomposition method (ADM) to approximate the analytical solution~\cite{Adomian1995}.
% TODO: Agregar una referencia sobre el teorema de Taylor.
The ADM yields a series expansion in terms of Adomian polynomials, which, combined with Taylor's theorem, provides an error bound for the ADM approximation~\cite{Umesh2021}.

In addition to the above, our contribution is to compare the ADM with the RK5(4)7M method across multiple polytropic indices, as demonstrated in the convergence tables below.
...
Firstly, we shortly recall the Adomian decomposition method (see [4,5,23]) for understanding a construction of the general formula for the differential transformation components which is based on the Adomian polynomials.
...
with prescribed auxiliary conditions, where $u$ is the unknown function, $L$ is the highest order derivative which is assumed to be easily invertible, $R$ is a linear differential operator of order less than $L, N u$ represents the nonlinear terms, and $g$ is the source term. Applying the inverse operator $L^{-1}$ to both sides of Eq. (7) and using the given conditions we obtain

$$
u=v-L^{-1}(R u)-L^{-1}(N u)
$$

where the function $v$ represents the terms arising from integrating the source term $g$ and from using the auxiliary conditions. The standard Adomian method defines the solution $u$ by an infinite series

$$
u=\sum_{n=0}^{\infty} u_{n} .
$$

The nonlinear term $N u$ is usually expressed as a nonlinear function $f(u)$. Generally, the nonlinear function $f(u)$, assumed to be an analytic function, is decomposed as follows:

$$
f(u)=\sum_{n=0}^{\infty} A_{n}\left(u_{0}, u_{1} \ldots u_{n}\right),
$$

where $A_{n}$ are the appropriate Adomian polynomials which are generated according to algorithm determined, for example, in [4,5,23]. The Adomian polynomials $A_{n}$ depend, of course, on the particular nonlinearity and are calculated by the general formula


\begin{equation*}
A_{n}\left(u_{0}, u_{1} \ldots u_{n}\right)=\frac{1}{n!} \frac{d^{n}}{d \lambda^{n}}\left[f\left(\sum_{k=0}^{\infty} \lambda^{k} u_{k}\right)\right]_{\lambda=0}, \quad n \geq 0 . \tag{8}
\end{equation*}
...
$$
\begin{aligned}
& A_{0}=f\left(u_{0}\right) \\
& A_{1}=u_{1} f^{\prime}\left(u_{0}\right) \\
& A_{2}=u_{2} f^{\prime}\left(u_{0}\right)+\frac{u_{1}^{2}}{2!} f^{\prime \prime}\left(u_{0}\right) \\
& A_{3}=u_{3} f^{\prime}\left(u_{0}\right)+u_{1} u_{2} f^{\prime \prime}\left(u_{0}\right)+\frac{u_{1}^{3}}{3!} f^{\prime \prime \prime}\left(u_{0}\right) \\
& \vdots
\end{aligned}
$$

The components $u_{0}, u_{1}, u_{2}, \ldots$ are thus determined as follows:

$$
\begin{aligned}
& u_{0}=v \\
& u_{k+1}=-L^{-1} R u_{k}-L^{-1} A_{k}, \quad k \geq 0,
\end{aligned}
$$

where $u_{0}$ is referred to as the zeroth component.
