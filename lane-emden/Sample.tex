...
\title[fsagg]{Solución númerica de lane-endem}

\author[1]{Matihus Alberth Molina Larios}\orcid{poner algo}
\author[2]{Alexandre Girouard}\orcid{0000-0001-8823-831X}

\authormark{Bruno Colbois \textit{et al}.}

\address[1]{\orgname{Universidad Nacional Mayor de San Marcos, Facultad de Ciencias Matemáticas}, \orgaddress{\street{Calle Germán Amézaga 375, Lima, 15081, Peru}, \postcode{CH-2000 Neuchâtel}, \country{Perú}};\email{matihus.molina@unmsm.edu.pe}}
...
\abstract{Dada una hipersuperficie suave y compacta $M$ con frontera
$\Sigma=\partial M$, demostraremos la existencia de una sucesión
$M_{j}$ de hipersuperficies con la misma frontera que $M$, tal que
cada valor propio Steklov $\sigma_{k}\left(M_{j}\right)$ tiende a
cero cuando $j$ tiende al infinito.
Las hipersuperficies $M_{j}$ se obtienen a partir de $M$ mediante una
perturbación local cerca de un punto de su frontera.
Sus volúmenes y diámetros son arbitrariamente cercanos a los de $M$,
mientras que las curvaturas principales de la frontera permanecen
inalteradas.}
...
Vea \cite{GP17,legacy} for background on this problem.
...
In \cite{CGG17} a more restrictive constraint was imposed by requiring the manifold $M$ to be a submanifold of $\mathbb{R}^m$ with prescribed boundary $\Sigma=\partial M\subset\mathbb{R}^m$.
In this context an upper bound for $\sigma_k$ was given in terms of $\Sigma$ and of the volume of $M$. The authors were unable at the time to give a lower bound and they raised the question of whether one exists, or if instead, arbitrarily small eigenvalues are possible. The goal of this paper is to answer that question.
...
\begin{remark}\label{remark:regularitycodim}
 Teorema~\ref{thm:main} holds in arbitrary positive codimension and ambient space. We decided to state it for hypersurfaces in $\mathbb{R}^{n+1}$for the sake of notational simplicity. Note also that Teorema~\ref{thm:main} certainly holds under weaker regularity asumptions.
\end{remark}
...
By construction (see Section \ref{section:perturbation}), each manifold $M_j$ coincides with $M$ in a neighborhood $\Omega_j$ of its prescribed boundary $\Sigma$.
  ...
  The asymptotic behavior of $\sigma_k(M_j)$ as $k\to\infty$ is therefore independent of $j$. That is, for each $j_1, j_2\in\mathbb{N}$ the following holds:
    $\sigma_k(M_{j_1})-\sigma_k(M_{j_2})=O(k^{-\infty})$; see~\cite[Lema~2.1]{GPPS}. In particular, the limits of $\sigma_k(M_j)$ as $j\to\infty$ and as $k\to\infty$ do not commute.
\end{remark}

\vspace*{-10pt}%
\subsection{La Estrategia de la Demostración}
For eigenvalues of the Laplace operator, it is well known that one can obtain arbitrarily small eigenvalues by constructing thin Cheeger dumbbells in the interior of the manifold; see \cite{Chavel,CoursColboisMtl}. This strategy does not work for Steklov eigenvalues. For Steklov eigenvalues, it is possible to obtain arbitrarily small eigenvalues by creating thin channels, but this involves deformation of the boundary or a perturbation of the Riemannian metric in the interior of $M$; see \cite{GP10,CESG17}. In order to prove Teorema~\ref{thm:main}, we have to use a more elaborate strategy.
