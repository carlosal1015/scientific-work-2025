...
\begin{poster}{
    ...
    \headerbox{\bf\color{ctitle}Adomian decomposition method}{name=method,column=1,row=0,span=1}{
        ...
        \textbf{Definition (Power series)}
        \begin{equation*}
            Lu+Ru+Nu=g \quad\Longrightarrow\quad u=A+Bt+L^{-1}g-L^{-1}Ru-L^{-1}Nu,
        \end{equation*}
        where \(L\) is the highest order differential operator,
        \(R\) is the remainder of linear operator,
        \(N\) is the nonlinear operator,
        and \(g\) is the source term.
        The solution $u$ is decomposed into $\sum_{n=0}^{\infty}u_n$ with $u_0=A+Bt+L^{-1}g$. Then:
        \begin{equation*}
            \sum_{n=0}^{\infty}u_{n}=u_{0}-L^{-1}R\left(\sum_{n=0}^{\infty}u_{n}\right)-L^{-1}\left(\sum_{n=0}^{\infty}A_{n}\right).
        \end{equation*}
        Where $A_n$ are special Adomian polynomials.
        We use:
        \begin{equation*}
            L=\xi^{-2}(\diff{}{r})\xi^2(\diff{}{r}) \longrightarrow L^{-1}=\int_{0}^{\xi}\xi^{-2}\left(\int_{0}^{\xi}\xi^{2}(\cdot)\dl\xi\right)\dl\xi.
        \end{equation*}

        \textbf{Definition (Transformation)}

        \begin{equation*}
            Y(k)=\frac{1}{k!}\left.\diff[k]{y(x)}{x}\right|_{x=x_0}
            \quad\Longrightarrow\quad y(x)=\sum_{k=0}^{\infty}Y(k){(x-x_0)}^{k}.
        \end{equation*}
    ...
    \headerbox{\bf\color{ctitle}Differential Transformation method}{name=results,column=2,row=0,span=2}{
        \textbf{Theorem.} (Existence and uniqueness of Cauchy problem of Lane-Emden)
        Let $m\in\mathbb{N}$.
        If
        \begin{math}
            f\in
